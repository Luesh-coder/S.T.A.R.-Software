...
# Minimum change required to actually advance the last transmitted
# value. NOTE: packets are still sent every tick (to keep the ESP32's
# 500 ms PACKET_TIMEOUT from dropping us into IDLE); this threshold
# only gates whether last_tx_x/y advances, so sub-delta wobble
# averages flat instead of ratcheting.
...
# Velocity estimate for look-ahead prediction (pixels / frame)
box_vel_x = 0.0
box_vel_y = 0.0
    ...
    while True:
        ...
        if tracker is not None:
            ...
            if tracker_ok:
                ...
                if target_box is None:
                    ...
                    # --- velocity estimation ---
                    old_cx, old_cy = box_center(target_box)
                    new_cx, new_cy = box_center(new_box)
                    box_vel_x = (1 - VEL_SMOOTH_ALPHA) * box_vel_x + VEL_SMOOTH_ALPHA * (new_cx - old_cx)
                    box_vel_y = (1 - VEL_SMOOTH_ALPHA) * box_vel_y + VEL_SMOOTH_ALPHA * (new_cy - old_cy)

                    # --- predictive look-ahead: shift new_box forward by vel * k ---
                    w = new_box[2] - new_box[0]
                    h = new_box[3] - new_box[1]
                    pred_cx = new_cx + box_vel_x * VEL_PREDICT_K
                    pred_cy = new_cy + box_vel_y * VEL_PREDICT_K
                    pred_box = (pred_cx - w / 2, pred_cy - h / 2, pred_cx + w / 2, pred_cy + h / 2)

                    pred_box = apply_box_corner_deadzone(target_box, pred_box, BOX_CORNER_DEADZONE_PX)
                    alpha = BOX_EMA_ALPHA_SLOW if still_frames >= STILL_FRAMES_REQUIRED else BOX_EMA_ALPHA_FAST
                    target_box = blend_box(target_box, pred_box, alpha)
            else:
                lost_frames += 1
        ...
        if run_yolo:
            ...
            if target_box is None:
                ...
                if best_match is not None and best_iou >= IOU_KEEP_THRESH:
                    # Update velocity from YOLO correction
                    y_cx, y_cy = box_center(best_match)
                    t_cx, t_cy = box_center(target_box)
                    box_vel_x = (1 - VEL_SMOOTH_ALPHA) * box_vel_x + VEL_SMOOTH_ALPHA * (y_cx - t_cx)
                    box_vel_y = (1 - VEL_SMOOTH_ALPHA) * box_vel_y + VEL_SMOOTH_ALPHA * (y_cy - t_cy)
                    ...
                    if lost_frames >= MAX_LOST_FRAMES:
                        # Target lost — reset state. Do NOT write an idle
                        # packet here: the fixed-rate sender below will
                        # emit it on the next tick, keeping all UART
                        # traffic under the SEND_HZ budget and preventing
                        # tx_idle_count double-counts.
                        target_box = None
                        tracker = None
                        tracker_ok = False
                        lost_frames = 0
        ...
        if now - last_send_t >= SEND_PERIOD:
            ...
            if target_box is not None:
                ...
                # Clamp to [-1, +1] — ESP32 clamps too, but be explicit so
                # no NaN/huge value can ever hit the UART if the box goes
                # briefly out-of-frame due to tracker drift.
                ...
                # If target is still, hard-freeze the output at the last
                # transmitted value. This comes AFTER the deadzone check
                # so a still target inside the deadzone still reports 0.
                if still_frames >= STILL_FRAMES_REQUIRED:
                    out_x = last_tx_x
                    out_y = last_tx_y

                # Commit new transmit values. We ALWAYS send a packet on
                # every tick so the ESP32's 500 ms PACKET_TIMEOUT doesn't
                # drop us into IDLE. MIN_SEND_DELTA only gates whether
                # last_tx_x/y actually advances — sub-delta wobble then
                # averages flat instead of ratcheting.
