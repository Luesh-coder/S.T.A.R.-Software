"""
S.T.A.R. – Raspberry Pi CM5 Vision & Tracking  (v11)

Changes vs. v10:
  - Spotlight-drift fix: when the MOSSE tracker latches onto the bright
    spotlit patch on the floor, the system used to wait MAX_LOST_FRAMES
    (~20 frames) before resetting. v11 detects the drift the same frame:
      * If the ROI YOLO call sees nobody, retry on the full frame so the
        actual person can be found.
      * If the tracker box has near-zero IoU with every YOLO detection but
        YOLO does see person(s), snap to the person nearest to where we
        last had a confirmed lock and re-init the tracker immediately.
  - REINIT_IOU_THRESH lowered 0.70 → 0.55 so the tracker is re-seeded
    more eagerly while YOLO and the tracker still mostly agree, which
    prevents slow drift from accumulating in the first place.
"""
...
# =============================================================
#  TRACKING ACCURACY TUNING
#  These are the Pi-side knobs. Adjust them to change what
#  norm_x / norm_y values get sent to the ESP32 gimbal.
#  For ESP32-side knobs (direction, speed, overshoot) see
#  STAR_ESP32_APP_AND_TRACKING_V2.ino → TRACKING ACCURACY TUNING.
# =============================================================

# --- DETECTION ---
# CONF_THRESH : confidence required to accept a YOLO person detection.
#   Raise → fewer false locks (good).
#   Lower → picks up targets at distance / partial occlusion.
CONF_THRESH = 0.55
PERSON_CLASS = [0]

# YOLO_EVERY_N_FRAMES : run YOLO every N frames; MOSSE tracker fills the gaps.
#   Lower (e.g. 2) → YOLO corrects the tracker more often → more accurate box.
#   Higher (e.g. 6) → faster FPS but tracker drifts more between corrections.
YOLO_EVERY_N_FRAMES = 2

# --- BOX SMOOTHING (affects how quickly the tracked bounding box moves) ---
# FAST alpha used when person is moving; SLOW alpha used when still.
#   Higher → box snaps to new position faster → snappier tracking.
#   Lower  → box moves more smoothly → less jitter but adds lag.
BOX_EMA_ALPHA_FAST = 0.55
BOX_EMA_ALPHA_SLOW = 0.08
BOX_CORNER_DEADZONE_PX = 2.0   # ignore corner wobble smaller than this many px

# --- OUTPUT SMOOTHING (directly scales the norm_x/norm_y sent to ESP32) ---
# FAST alpha used when person is moving; SLOW alpha used when still.
#   Higher → ESP32 receives snappier / more reactive coordinates.
#   Lower  → ESP32 receives smoother but laggier coordinates.
OUTPUT_EMA_ALPHA_FAST = 0.75
OUTPUT_EMA_ALPHA_SLOW = 0.10

# --- DEADZONES (stop the ESP32 from chasing tiny errors near frame centre) ---
# INNER : once inside this zone the output is held at 0 (servo holds).
# OUTER : output resumes only once error exceeds this (hysteresis).
# Raise INNER/OUTER → calmer hold, less micro-jitter from the gimbal.
# Lower INNER/OUTER → tighter re-centering, may cause jitter.
DEADZONE_INNER = 0.025
DEADZONE_OUTER = 0.045

# MIN_SEND_DELTA : suppress transmission if the coord changed less than this.
#   Raise → fewer UART packets, less micro-motion in the gimbal.
MIN_SEND_DELTA = 0.004

# --- STILLNESS FREEZE ---
# If the torso centre moves less than STILL_CENTER_DEADZONE_PX pixels for
# STILL_FRAMES_REQUIRED consecutive frames, output is frozen completely.
#   Raise DEADZONE_PX or lower FRAMES_REQUIRED → freeze triggers sooner.
STILL_CENTER_DEADZONE_PX = 3.0
STILL_FRAMES_REQUIRED = 6

# Target handling
MAX_LOST_FRAMES = 20
IOU_KEEP_THRESH = 0.08

# --- TRACKER ---
# v11: lowered 0.70 → 0.55 to re-seed the tracker more eagerly while
# YOLO and the tracker still mostly agree, preventing slow drift onto
# the spotlit patch from accumulating.
REINIT_IOU_THRESH = 0.55
...
# v11: max distance (as a fraction of frame width) between the last
# confirmed torso center and a YOLO candidate when snapping back from
# tracker drift. Stops us from jumping to an unrelated person across
# the frame during a real occlusion.
SNAP_BACK_MAX_DIST_FRAC = 0.5
...
def expand_and_clip_roi(box, frame_w, frame_h, scale):
    ...
    cx = (x1 + x2) * 0.5
    cy = (y1 + y2) * 0.5
    bw = max(abs(x2 - x1) * scale, 1.0)
    bh = max(abs(y2 - y1) * scale, 1.0)
    rx1 = int(clamp(cx - bw * 0.5, 0, frame_w - 2))
    ry1 = int(clamp(cy - bh * 0.5, 0, frame_h - 2))
    rx2 = int(clamp(cx + bw * 0.5, rx1 + 1, frame_w - 1))
    ry2 = int(clamp(cy + bh * 0.5, ry1 + 1, frame_h - 1))
...
# Velocity predictor: exponential-smoothed box velocity (px/frame)
box_vel = (0.0, 0.0, 0.0, 0.0)
BOX_VEL_ALPHA = 0.4   # how quickly velocity estimate adapts
...
# v11: cache of the most recent YOLO-confirmed torso center. Used as the
# anchor for the snap-back logic when the tracker has drifted off the
# person (e.g. onto the spotlit patch).
last_good_torso_center = None
...
try:
    while True:
        ...
        if tracker is not None:
            ...
            if tracker_ok:
                ...
                if target_box is None:
                    ...
                    blended = blend_box(target_box, new_box, alpha)
                    # Update velocity estimate
                    if prev_target_box is not None:
                        raw_vel = tuple(blended[i] - prev_target_box[i] for i in range(4))
                        box_vel = tuple(
                            (1 - BOX_VEL_ALPHA) * box_vel[i] + BOX_VEL_ALPHA * raw_vel[i]
                            for i in range(4)
                        )
                    prev_target_box = target_box
                    target_box = blended
            else:
                # Tracker lost — extrapolate using last known velocity
                if target_box is not None and any(abs(v) > 0.1 for v in box_vel):
                    target_box = tuple(target_box[i] + box_vel[i] for i in range(4))
                lost_frames += 1
        ...
        if run_yolo:
            ...
            # v11: ROI saw nobody → tracker has likely drifted off the person
            # (commonly onto the spotlit patch). Retry on the full frame so
            # the snap-back path below can find the actual person.
            if used_roi and not persons:
            ...
            if target_box is None:
                if persons:
                    ...
                    box_vel = (0.0, 0.0, 0.0, 0.0)
                    prev_target_box = target_box
                    last_good_torso_center = torso_center_from_box(target_box)
            else:
                ...
                if best_match is not None and best_iou >= IOU_KEEP_THRESH:
                    ...
                    # v11: tracker box has near-zero IoU with every YOLO
                    # detection. If YOLO does see person(s), treat the
                    # tracker as having latched onto a non-person feature
                    # (e.g. the spotlit patch) and snap to the person
                    # nearest to where we last had a confirmed lock. If we
                    # can't find a plausible candidate, fall through to the
                    # original lost-frames timeout.
                    snapped = False
                    if persons:
                        anchor = last_good_torso_center
                        if anchor is None:
                            anchor = box_center(target_box)
                        ax, ay = anchor
                        max_dist2 = (frame_w * SNAP_BACK_MAX_DIST_FRAC) ** 2
                        best_snap = None
                        best_d2 = 1e18
                        for _conf, pbox in persons:
                            bcx, bcy = box_center(pbox)
                            d2 = (bcx - ax) ** 2 + (bcy - ay) ** 2
                            if d2 < best_d2:
                                best_d2 = d2
                                best_snap = pbox
                        if best_snap is not None and best_d2 <= max_dist2:
                            target_box = best_snap
                            tracker = make_tracker(TRACKER_TYPE)
                            tracker.init(frame, box_to_tracker_rect(target_box))
                            tracker_ok = True
                            lost_frames = 0
                            ...
                            last_good_torso_center = torso_center_from_box(target_box)
                            snapped = True
                            print("[TARGET] re-acquired after tracker drift")
